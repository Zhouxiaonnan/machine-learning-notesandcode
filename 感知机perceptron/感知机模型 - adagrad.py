...
def Perceptron(train_data, train_label, itertime):
    ...
    # 初始化adagrad中的分母项（均方根项）
    # 均方根:root-mean-square(rmt)
    wGrmt = np.zeros(np.shape(train_data)[1])
    bGrmt = 0
    ...
    for t in range(itertime):
        ...
        # 循环结束后，更新均方根项
        wGrmt = np.sqrt(np.square(wGrmt) + np.square(wGs))
        bGrmt = np.sqrt(np.square(bGrmt) + np.square(bGs))
        
        # 对w和b进行迭代，后面1e-8是为了防止除0
        w = w - h * wGs / (wGrmt + 1e-8)
        b = b - h * bGs / (bGrmt + 1e-8)
